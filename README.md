...
После запуска `main.py` выберите режим:

После выбора моделей программа предлагает режим обработки:
- **1** — полный цикл: распознавание, LLM-ответ и озвучка
- **2** — только распознавание речи, без запросов к LLM и TTS

Режим **2** предназначен для честного сравнения моделей STT. В этом режиме поля `llm_sec` и `tts_sec` в `metrics.csv` равны `0.00`.
Пути к моделям задаются в программе и не запрашиваются при запуске. Результаты каждого выбранного движка сохраняются в `metrics.csv`:

```text
engine,audio_sec,asr_sec,rtf,llm_sec,tts_sec,e2e_sec
```

### Визуализация метрик

Для построения графиков и сводки по движкам выполните:

```bash
python src/visualize_metrics.py
```

Скрипт сравнивает следующие показатели:

- средние задержки ASR, LLM, TTS и E2E для каждого движка;
- средний RTF для каждого движка;
- E2E-задержку каждого отдельного теста;
- зависимость длительности аудио от E2E-задержки.

Графики сохраняются в `metrics_visualization.png`, а табличная сводка выводится в консоль. Пути к входному CSV и выходному PNG можно изменить параметрами `--input` и `--output`.
